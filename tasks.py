...
LAYERS_DIR = 'layertest/'
REQUIREMENTS_FILE = 'requirements.txt'
SRC_ZIP = 'lambda_code.zip'
LAMBDA_FUNCTION = 'scan_dependabot_prs'
LAMBDA_LAYER = 'new-layer'
...
def build_layer(c, versions='python3.8', output='lambda-layer'):
    ...
    print("Checking requirements to build the layers".format(output))
    ...
    with c.cd(LAYERS_DIR):
        c.run('zip -r {}.zip python'.format(output))


@task(help={"zipfile": "Output zip. Defaults to 'lambda-layer.zip'",
            "name": "AWS Lambda Layer to create/publish to.",
            "description": "Description of layer on AWS",
            "runtimes": "Compatible runtimes for this package 'python3.7,python3.8'"})
def publish_layer(c, name, zipfile='lambda-layer.zip', runtimes='python3.8', description=None):
    """
...
@task(help={"name": "Lambda function to publish to. Defaults to lambda_dependabot",
            "srcfile": "Python src file containing the lamdba handler. Defaults to lambda_function.py"})
def publish_code(c, name="lambda-dependabot", srcfile="lambda_function.py"):
    """
